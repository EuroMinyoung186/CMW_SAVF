...
base_path = '/watermark/HDTF_SAVE/audio_only'
base_path2 = '/watermark/HDTF_SAVE/vc_result'
output_path = '/watermark/Interspeech_Test/STFT_DWT_FACIAL_SHORT_RESHAPE/save/VC_EXCHANGE'
output_path2 = '/watermark/Interspeech_Test/STFT_DWT_FACIAL_SHORT_RESHAPE/save/VC_MASK'
...
for filepath in filepaths:
    ...
    replace_path2 = os.path.join(base_path2, last_name)
    output_path_replace = os.path.join(output_path, last_name)
    output_path_replace2 = os.path.join(output_path2, last_name)
    ...
    wavfile2 = sorted(os.listdir(replace_path2))   
    wavfile2 = [os.path.join(replace_path2, f)  for f in wavfile2 if f.endswith('.wav')]
    ...
    for file in tqdm(wavfile):
        ...
        possible_files = [f for f in wavfile2 if f != file]
        ...
        os.makedirs(output_path_replace, exist_ok=True)
        os.makedirs(output_path_replace2, exist_ok=True)
        audio_output = os.path.join(output_path_replace, f"{base_name}.wav")
        mask_output = os.path.join(output_path_replace2, f"{base_name}.npy")
        
        print(f"  새 오디오 저장: {audio_output}")
        soundfile.write(audio_output, new_audio, sr, format='WAV')
