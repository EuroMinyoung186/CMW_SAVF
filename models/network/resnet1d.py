...
class ResNet1D(nn.Module):
    # Resnet 18 [2, 2, 2, 2]
    def __init__(self, block, output_channel):
        ...
        self.fc = nn.Linear(512*self.expansion, output_channel)
    ...
    def forward(self,x):
        ...
        x = x.reshape(x.shape[0], -1) #B*32, 512
        x = self.fc(x) #B*32, 256
...
def CreateResNet1D(output_channel = 256):
    return ResNet1D(block, output_channel=output_channel)